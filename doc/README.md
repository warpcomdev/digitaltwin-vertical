...
- Una **caracterización estadística** del comportamiento de algún servicio público o métrica de una ciudad (ejemplos: intensidad del tráfico, probabilidad de congestión de las principales vías, etc), que proporciona información sobre los patrones habituales que sigue dicho servicio en diferentes momentos o diferentes zonas de la ciudad.

  A esta caracterización la denominamos **identidad** de la ciudad.

- Un **modelo matemático** que predice cambios en dicha caracterización, a largo plazo, como consecuencia de modificaciones urbanísticas, regulatorias, etc, que puedan afectar al comportamiento de los ciudadanos.

  Este modelo utiliza un conjunto de supuestos (**escenarios**) para calcular variaciones en la caracterización de la ciudad (**simulación**).

## Datasets

Tanto el análisis estadístico como el aprendizaje predictivo que se realizan en este proyecto, se basan en el procesamiento de un conjunto de **datasets**. Cada dataset consiste en una **Serie temporal monovariable georeferenciada** que contiene información histórica sobre una métrica particular de la ciudad. Cada elemento de cada dataset proporciona:
...
**NOTA**: Con el objetivo de optimizar el uso de recursos (CPU, memoria, espacio en disco), no es necesario que los datasets sean tablas concretas de la base de datos, o ficheros concretos en el disco duro del equipo. Un dataset puede ser simplemente una query `postgreSQL`, una agregación de `mongoDB`, o un proceso `spark` que obtiene los datos necesarios desde las bases de datos o datalakes del cliente.

Los siguientes apartados enumeran los datasets soportados por la aplicación actualmente.
...
### Frecuencia de paso de tte. público
